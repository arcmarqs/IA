...
public class Guard_greedy2{
  ...
  public static void print(boolean to_visit[],int n_ret){
    for(int i=1;i<=n_ret;i++) System.out.print(to_visit[i]+" ");
    System.out.println();
  }
    ...
    while(!pq.isEmpty()){
      ...
      for(int i = 1;i<=size;i++){
        if(Vertmap.get(rect1.vert_ids[i]) == null) continue;
          ...
          aux = Vertmap.get(rect1.vert_ids[i]);
          max = aux.n_rects;
        }
      }
